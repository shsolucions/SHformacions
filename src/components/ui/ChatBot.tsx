...
export function ChatBot({ onClose }: ChatBotProps) {
  ...
  useEffect(() => {
    ...
    if (!last || last.role !== 'model' || contactSaved || messages.length < 4) return;
    const text = last.text.toLowerCase();
    if ((text.includes('telèfon') || text.includes('correu')) && text.includes('pressupost')) {
  ...
  const handleSaveContact = async (phone: string, email: string) => {
    ...
    const fullChat = messages.map((m) => `${m.role === 'user' ? 'Client' : 'Assistent'}: ${m.text}`).join('\n---\n');
    ...
    await saveConversationToSheet({
      ...
      summary: userText.slice(0, 300), courses: coursesFound, fullChat,
    });
    setMessages((prev) => [...prev, {
      role: 'model',
